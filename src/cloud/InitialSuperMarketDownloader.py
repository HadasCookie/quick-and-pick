...
def setup_driver():
    ...
    driver.temp_download_dir = temp_dir
    return driver
...
def download_shufersal(chain_id, store_ids):
    ...
    try:
        ...
        )).select_by_visible_text("PricesFull")
        ...
        while True:
            ...
            for row in rows:
                try:
                    ...
                    parts = filename.replace(".gz", "").replace("PriceFull", "").split("-")
...
def download_victory(chain_id, store_ids):
    ...
    try:
        ...
        Select(driver.find_element(By.ID, "MainContent_fileType")).select_by_value("pricefull")
        time.sleep(1)
        ...
        for row in rows:
            try:
                ...
                match = re.match(r"PriceFull(\d+)-(\d+)-", filename)
                if not match:
                    continue
...
def download_tivtaam(chain_id, store_ids):
    ...
    try:
        ...
        # Wait for search bar and search "PriceFull"
        ...
        for i in range(len(rows)):  
            try:
                ...
                if not filename.endswith(".gz") or "PriceFull" not in filename:
                    continue

                # Parse chain_id and store_id
                match = re.match(r"PriceFull(\d+)-(\d+)-(\d+)\.gz", filename)
                if not match:
                    continue
        ...
        print(f" Found {len(latest_files)} files.")

        downloaded = set()
        for store_id, (filename, _, timestamp) in latest_files.items():
...
def extract_gz_files(download_dir, chain_name):
    ...
    chain_folder = f"{chain_name}/PricesFull"
...
# === Main Function to Download Initial Data ===
def download_initial_supermarket_data():
